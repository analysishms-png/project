...
class ServerMasterExport {
    ...
    public function getData() {
        return DB::table('server_mast')->where('propertyid', $this->propertyid)->orderBy('name', 'ASC')->get();
    }
    public function download() {
        $data = $this->getData();
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Server Master');
        ...
        $sheet->mergeCells('A2:C2'); $sheet->setCellValue('A2', 'Server Master');
        $sheet->getStyle('A2')->applyFromArray(['font' => ['bold' => true, 'size' => 11], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER]]);
        $currentRow = 4;
        foreach (['Sn.', 'Name', 'Active'] as $i => $h) { $sheet->setCellValue(['A','B','C'][$i] . $currentRow, $h); }
        ...
        header('Content-Disposition: attachment; filename="Server_Master.xlsx"');
