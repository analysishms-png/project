...
| PERF-02 | Zero eager loading (`->with()`/`->load()`) in controllers | HIGH | PARTIALLY DONE 2026-08-17 — hot paths batched (Daily Report 224→66, reservedrooms 880→5, lookuproomtype 310→4, roominventory 110→3, getindex loops; 2026-08-17: Pos possalebillsettle mergedBills 2q→1 per bill, Banquet tax-name lookups 1q→1 per booking, focc_reportfetch depart 1q→1 per report); remaining N+1 sites still OPEN |
