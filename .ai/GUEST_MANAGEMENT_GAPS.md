...
|---|---|---|---|---|---|
| GM-01 | **Wake-up call booking** (`FrmGuestWakeUp`) | `GuestWakeUp` table: DocID, VNo, RoomNo, RoomCat, Extension, RemReqd, FoodOrd, OtherReq, WDate, WTime, GuestProf, FolioNo — full CRUD linked to RoomOcc/GuestFolio | ❌ MISSING — no table, no route, no view (grep `wakeup` → 0 hits in app/routes/views) | P1 | Implement module: table + CRUD + room/folic-linked + report. Modern replacement could be guest-portal "wake-up request", but desk-side screen is the legacy contract. |
| GM-02 | **House guest messages** (`FrmHouGuestMsg`) | `GuestMessage` table: RoomNo, RoomCat, Caller, Telephone, Message, RecdDate, RecdTime, GuestProf, FolioNo — insert/update/delete/print | ❌ MISSING — no table/route/view | P1 | Implement: take message at desk for an in-house guest, deliver/notify, log with audit fields. |
...
| GM-07 | **Guest master browse/list screen** | `frmGuestInfo` = searchable master list (CODE, NAME ordered, drill to full profile) | ⚠️ PARTIAL — guest history lookup exists (`Fetch@guesthistory` by name/mobile, used in walkin screen), profile add/edit exists, but no dedicated **Guest Master** list page with search + visit history | P2 | Build a read-only Guest Master page: search by name/mobile/email, list past stays (via roomocc/guestfolio join), link to profile edit. |
