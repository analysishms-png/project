...
@section('main-container')
        ...
        <div class="container-fluid">
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Gate Pass IN - Word</h4>
                        </div>
                        <div class="card-body">
                            <div id="alertMessage"></div>
                            
                            <form id="gatePassInForm">
                                @csrf
                                <input type="hidden" id="gatepassin_id" name="gatepassin_id">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" id="date" class="form-control" value="{{ date('d-m-Y') }}" readonly>
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select id="type" name="type" class="form-control" required>
                                            ...
                                            <option value="InWord">In Word</option>
                                        </select>
                                    </div>
                                    ...
                                    <!-- Word Status (for edit) -->
                                    <div class="col-md-3" id="wordstatus_div" style="display: none;">
                                    ...
                                    <div class="col-md-12 mt-3">
                                        ...
                                        <button type="button" class="btn btn-secondary" id="resetBtn" onclick="resetFormIn()">
            ...
            <div class="row mt-3">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Gate Pass IN Records</h4>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped verticle-middle" id="gatePassInTable">
                                    <thead>
                                        <tr>
                                            <th>Pass No</th>
                                            <th>Date</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Form Submit
            $('#gatePassInForm').submit(function(e) {
                e.preventDefault();
                ...
                let formData = {
                    ...
                    remark: $('#remark').val(),
                    wordstatus: $('#wordstatus').val(),
                    _token: $('input[name="_token"]').val()
                };

                let url = '{{ route("inwarddetails.store") }}';
                let method = 'POST';
                let gatepassinId = $('#gatepassin_id').val();

                if (gatepassinId) {
                    url = '/inwarddetails/' + gatepassinId;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        showAlertIn('success', response.message);
                        resetFormIn();
                        loadGatePassIns();
                    },
                    error: function(xhr) {
        ...
        function editGatePassIn(id) {
            $.ajax({
                url: '/inwarddetails/' + id + '/edit',
                type: 'GET',
                success: function(response) {
                    $('#gatepassin_id').val(response.sn);
                    ...
                    $('#wordstatus').val(response.wordstatus);
                    $('#wordstatus_div').show();
                ...
                error: function(xhr) {
                    showAlertIn('danger', 'Error loading gate pass data');
                }
            });
        }

        function deleteGatePassIn(id) {
            if (!confirm('Are you sure you want to delete this gate pass?')) {
                return;
            }

            $.ajax({
                url: '/inwarddetails/' + id,
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                ...
                error: function(xhr) {
                    showAlertIn('danger', 'Error deleting gate pass');
                }
            });
        }

        function loadGatePassIns() {
            $.ajax({
                url: '{{ route("inwarddetails.index") }}',
                type: 'GET',
                data: { ajax: 1 },
                success: function(response) {
                    console.log(response);
                    $('#gatePassInTableBody').html(response.html);
                }
            });
        }

        function showAlertIn(type, message) {
            let alertHtml = `
