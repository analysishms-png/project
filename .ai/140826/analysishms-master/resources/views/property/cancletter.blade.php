...
<head>
    ...
    <title>Cancel Letter Folio {{ $data->foliono }} Name {{ $data->GuestName }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="ref-no"><b>Reservation No:</b> {{ $data->BookNo }}/{{ date('d-M-Y', strtotime($curdate)) }}</div>
    ...
    <table style="width: 100%; border-collapse: collapse;">
        ...
        <tr>
            ...
            <th style="text-align: left; padding: 8px;">Cancel Date</th>
            <td style="padding: 8px;">{{ date('d-M-Y', strtotime($data->CancelDate)) }}</td>
            <th style="text-align: left; padding: 8px;">‎ </th>
            <td style="padding: 8px;">‎ </td>
        </tr>
    </table>

    <p><b>Sub: Cancel Letter for Ref. No.:</b> {{ $data->BookNo }}/{{ date('d-M-Y', strtotime($curdate)) }}</p>

    <p>Dear Guest,</br> With reference to the above subject and the Reservation No. {{ $data->BookNo }}/{{ date('d-m-Y', strtotime($curdate)) }}
        you are being initimated that as per Cancellation Policy you are requested to pay the Cancellation Charges as details given below:</p>
