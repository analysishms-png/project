...
@section('main-container')
                    ...
                    <div class="card">
                        <div class="card-body walkin">
                            ...
                            <form class="walkin-form" id="walkinform" name="walkinform"
                                ...
                                <input type="hidden" name="firstgrpsno" value="{{ $data[0]->Sno }}" id="firstgrpsno">
                                <input type="hidden" name="maxgrpsno" value="{{ $data[count($data) - 1]->Sno }}" id="maxgrpsno">
                                <div class="row">
                                    <div class="col-md-6">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="input-group">
                                                            <input type="time" value="{{ date('H:i') }}" id="checkintime"
                                                    ...
                                                    <td style="display: {{ $data[0]->MarketSeg == 'Travel Agency' || $data[0]->MarketSeg == 'Travel Agency (OTA)' ? 'block' : 'none' }};"
                                                        ...
                                                        <select id="travel_agent" name="travel_agent" class="form-control low">
                                                            ...
                                                            @foreach ($travel_agent as $list)
                                                                <option data-comp_type="{{ $list->comp_type }}" value="{{ $list->sub_code }}" {{ $data[0]->TravelAgency == $list->sub_code ? 'selected' : '' }}>{{ $list->name }}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <td>
                                ...
                                <div>
                                    ...
                                    <table class="table-hover walkin-multi table-responsive" id="gridtaxstructure">
                                        ...
                                        <tbody>
                                            @foreach ($data as $index => $dbrow)
                                                <tr class="data-row">
                                                    <td>
                                                        <input type="hidden" name="grpsno{{ $dbrow->Sno }}" value="{{ $dbrow->Sno }}" id="grpsno{{ $dbrow->Sno }}">
                                                    ...
                                                    <td>
                                                        <input type="text" name="rate{{ $dbrow->Sno }}" id="rate{{ $dbrow->Sno }}" oninput="checkNumMax(this, 10); handleDecimalInput(event);" class="form-control ratechk sp" value="{{ $dbrow->Tarrif }}" required {{ $dbrow->planedit == 'Y' ? 'readonly' : '' }}>
                                                    </td>
                                                    <td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '.cat_code_class', function() {
                ...
                xhrRooms.onreadystatechange = function() {
                    if (xhrRooms.readyState === 4 && xhrRooms.status === 200) {
                        ...
                        var availableOptions = $(`#roommast${trno} option`).map(function() {
                            return $(this).val();
                        }).get();

                        if (exrval != '' && availableOptions.includes(exrval)) {
                            $(`#roommast${trno}`).val(exrval);
                        } else {
                            $(`#roommast${trno}`).val('');
                        }
            ...
            setTimeout(() => {
                if ($('#complimentry').is(':checked')) {
                    handleComplimentary($('#complimentry'));
                }
            }, 500);
        ...
        function getTrailingIndexFromValue(value) {
            if (!value) {
                return null;
            }
        ...
        function replaceTrailingIndex(value, oldIndex, newIndex) {
            if (!value || oldIndex === null || newIndex === null) {
                return value;
            }

            return String(value).replace(new RegExp(oldIndex + '$'), newIndex);
        }

        function getContainerIndex(container) {
            const currentId = container.getAttribute('id');
            const containerIndex = getTrailingIndexFromValue(currentId);
            if (containerIndex !== null) {
                return containerIndex;
            }

            const indexedElement = container.querySelector('[id],[name],[data-target],[data-sn]');
            if (!indexedElement) {
                return null;
            }

            return getTrailingIndexFromValue(indexedElement.id) ??
                getTrailingIndexFromValue(indexedElement.name) ??
                getTrailingIndexFromValue(indexedElement.getAttribute('data-target')) ??
                getTrailingIndexFromValue(indexedElement.getAttribute('data-sn'));
        }

        function renameIndexedAttributes(element, oldIndex, newIndex) {
            ['id', 'name', 'data-target', 'data-sn', 'for', 'aria-controls', 'aria-labelledby'].forEach(function(attributeName) {
                const attributeValue = element.getAttribute(attributeName);
                if (!attributeValue) {
                    return;
                }

                element.setAttribute(attributeName, replaceTrailingIndex(attributeValue, oldIndex, newIndex));
            });
        }

        function reserializeRoomRows() {
            $('#gridtaxstructure tbody tr.data-row').each(function(position) {
                const newIndex = position + 1;
                const oldIndex = getContainerIndex(this);

                $(this).find('[id],[name],[data-target],[data-sn],[for],[aria-controls],[aria-labelledby]').each(function() {
                    renameIndexedAttributes(this, oldIndex, newIndex);
                });
            });
        }

        function reserializeGuestCloneRows() {
            $('[id^="cloneit"]').each(function(position) {
                if (position === 0) {
                    this.id = 'cloneit';
                    return;
                }
        ...
        function reserializeWalkinRows() {
            reserializeRoomRows();
            reserializeGuestCloneRows();
        }

        $(document).ready(function() {
            reserializeWalkinRows();
        });
