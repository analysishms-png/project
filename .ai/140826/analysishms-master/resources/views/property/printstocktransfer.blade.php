...
<html lang="en">
@php
    $first = $stockTransferData->first();
@endphp

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Stock Transfer - Print</title>
    <style>
        @if ($first->delflag == 'Y')
            body::after {
                content: "CANCELLED";
...
<body>
    @if ($stockTransferData->count() > 0)
        ...
        <table class="items-table">
            ...
            <tbody>
                @php
                    ...
                    $totalAmount = 0;
                @endphp
                @foreach ($stockTransferData as $row)
                    <tr>
                        <td>{{ $row->sno }}</td>
                        <td>{{ $row->ItemName }}</td>
                        <td style="text-align: right;">{{ number_format($row->ReceivedQty, 3) }}</td>
                        <td style="text-align: center;">{{ $row->Unit }}</td>
                        <td style="text-align: right;">{{ number_format($row->Rate ?? 0, 2) }}</td>
                        <td style="text-align: right;">{{ number_format($row->Amount ?? 0, 2) }}</td>
                    </tr>
                    @php
                        $totalQuantity += $row->ReceivedQty;
        ...
        <div class="footer">
            <div class="signature">
                ...
                <div class="signature-line">Transferred By</div>
            </div>
            <div class="signature">
    ...
    @else
        <div style="text-align: center; margin-top: 50px;">
            <h3>No Data Found</h3>
            <p>Stock Transfer data not available.</p>
        </div>
    @endif
