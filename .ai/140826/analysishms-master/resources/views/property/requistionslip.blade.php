...
@section('main-container')
        ...
        <div class="container-fluid">
            <div class="row justify-content-center">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <form class="form" action="{{ route('requisitionslipsubmit') }}" name="requistionslipform"
                                id="requistionslipform" method="POST" enctype="multipart/form-data">
                                @csrf
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="godown" id="godown" required>
                                                <option value="">Select</option>
                                                @foreach ($godown as $item)
                                                    <option value="{{ $item->scode }}"
                                                        {{ $enviroinv->purchasegodown == $item->scode ? 'selected' : '' }}>
                                                        {{ $item->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('godown')
                                            ...
                                            @foreach ($data as $row)
                                                <tr>
                                                    ...
                                                    <td class="ins">
                                                        @if ($superwiser == '1')
                                                            <a
                                                                href="updaterequisitionslip/{{ $row->vno }}/{{ $row->vprefix }}/{{ $row->vtype }}">
                                                                <button class="btn btn-success editBtn update-btn btn-sm">
                                                                    <i class="fa-regular fa-pen-to-square"></i>
                                                                    {{ $row->veryfiuser != '' ? 'Verify' : 'Edit' }}
                                                                </button>
                                                            </a>
                                                        ...
                                                        @endif
                                                        <a href="{{ url('printrequistionslip/' . $row->docid) }}"
                                                            target="_blank">
                                                            <button class="btn btn-primary btn-sm"><i
                                                                    class="fas fa-print"></i> Print</button>
                                                        </a>
                                                        <a href="requisitionslipdelete/{{ $row->docid }}"><button
    ...
    <script>
        $(document).ready(function() {
            ...
            // DataTable Initialization for Requisition Slip Table
