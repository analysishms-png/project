...
@section('main-container')
        ...
        <div class="container-fluid">
            <div class="row justify-content-center">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <form class="form" name="bookingsourceform" id="bookingsourceform" action="{{ route('bookingsourcestore') }}"
                                method="POST">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                                        <label class="col-form-label" for="name">Booking Source Name</label>
                        ...
                        <div class="d-flex gap-2 px-1 pb-2 pt-1">
                            <button type="button" onclick="window.location.href='{{ route('bookingsource.export') }}'" class="btn btn-success btn-sm">Excel</button>
                            <button type="button" onclick="window.open('{{ route('printbookingsource') }}','_blank')" class="btn btn-info btn-sm text-white">Print</button>
                        </div>

                        <div class="table-responsive">
                            <table id="bookingsource"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $row)
                                        <tr>
                                            ...
                                            <td class="ins">
                                                <a
                                                    href="updatebookingsource?sn={{ base64_encode($row->sn) }}&bcode={{ base64_encode($row->bcode) }}">
                                                    <button
                                                        class="btn
                                                ...
                                                <a
                                                    href="deletebookingsource?sn={{ base64_encode($row->sn) }}&bcode={{ base64_encode($row->name) }}">
    ...
    <script>
        $(document).ready(function() {
            if (!$.fn.DataTable.isDataTable('#bookingsource')) {
                new DataTable('#bookingsource', { ordering: true, order: [], pageLength: 25 });
            }
        });
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        name.addEventListener('keyup', function() {
            ...
            xhr.open('POST', '/getbookingsourcenames', true);
