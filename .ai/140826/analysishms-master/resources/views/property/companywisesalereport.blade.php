...
@section('main-container')
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="container-fluid">
                                <div class="row">
                                    <div class="col-md-12">
                                        <h3>Company Wise Sales Report</h3>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row justify-content-around">
                                    ...
                                    <div class="">
                                        <div class="form-group">
                                            ...
                                            <?php // echo "<pre>"; print_r($company->toArray()); echo "</pre>"; ?>
                                            {{-- <label class="col-form-label">Party Name</label> --}}
                                            <button type="button" class="btn btn-outline-primary" id="companySelectBtn">Select Party Name</button>
                                            <div id="companyDropdown" style="display:none; position:absolute; background:#fff; border:1px solid #ccc; z-index:1000; padding:10px; max-height:300px; overflow:auto;">
                                                <div><input type="checkbox" id="allCompanies" checked> <label for="allCompanies"><b>All</b></label></div>
                                                @foreach ($partyName as $party)
                                                    <div><input type="checkbox" class="companyCheckbox" value="{{ $party->party }}" checked> <label>{{ $party->party }}</label></div>
                                                @endforeach
                                            </div>
                                            <input type="hidden" name="party_name" id="party_name" value="all">
                                        </div>
                                    </div>
                            ...
                            <div class="row table-responsive">
                                <table id="companywisesalereportdata"
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#companySelectBtn').on('click', function(e) {
                e.stopPropagation();
                $('#companyDropdown').toggle();
            });
            // Hide dropdown when clicking outside
            $(document).on('click', function(e) {
                if (!$(e.target).closest('#companyDropdown, #companySelectBtn').length) {
                    $('#companyDropdown').hide();
                }
            });
            // All checkbox logic
            $('#allCompanies').on('change', function() {
                if ($(this).is(':checked')) {
                    $('.companyCheckbox').prop('checked', true);
                } else {
                    $('.companyCheckbox').prop('checked', false);
                }
                updatePartyNameInput();
                $('#companyDropdown').hide();
                table.ajax.reload();
            });
            // Company checkbox logic
            $('.companyCheckbox').on('change', function() {
                var checkedBoxes = $('.companyCheckbox:checked');
                if (checkedBoxes.length === $('.companyCheckbox').length) {
                    $('#allCompanies').prop('checked', true);
                } else {
                    $('#allCompanies').prop('checked', false);
                }
                updatePartyNameInput();
               // $('#companyDropdown').hide();
                table.ajax.reload();
            ...
            function updatePartyNameInput() {
                var checkedBoxes = $('.companyCheckbox:checked');
                if ($('#allCompanies').is(':checked') || checkedBoxes.length === $('.companyCheckbox').length) {
                    $('#party_name').val('all');
                } else if (checkedBoxes.length > 0) {
                    var vals = checkedBoxes.map(function(){ return $(this).val(); }).get().join(',');
                    $('#party_name').val(vals);
                } else {
                    $('#party_name').val('');
                }
            ...
            var table = $('#companywisesalereportdata').DataTable({
                processing: true,
                serverSide: true,
                searching: true,
                paging: true,
                ordering: true,
                ajax: {
                    url: '{{ route('companywisesalereportdata') }}',
                    type: 'POST',
                    data: function (d) {
                        d.fromdate = $('#fromdate').val();
                        d.todate = $('#todate').val();
                        d.party_name = $('#party_name').val(); // comma-separated or 'all'
                        d._token = '{{ csrf_token() }}';
                    },
                    error: function (xhr) {
