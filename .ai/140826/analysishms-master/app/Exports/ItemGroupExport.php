...
class ItemGroupExport
{
    ...
    public function download()
    {
        ...
        $sheet->setCellValue('A2', 'Item Group');
        ...
        $currentRow = 4;
        $headers    = ['Sn.', 'Name', 'Type', 'Active YN'];
        $cols       = ['A', 'B', 'C', 'D'];
        ...
        foreach ($data as $i => $row) {
            ...
            $sheet->setCellValue('C' . $currentRow, $row->type    ?? '');
            $sheet->setCellValue('D' . $currentRow, $row->activeyn ?? '');
        ...
        header('Content-Disposition: attachment; filename="Item_Group.xlsx"');
