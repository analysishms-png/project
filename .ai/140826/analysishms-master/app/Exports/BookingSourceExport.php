...
class BookingSourceExport {
    ...
    public function getData() {
        return DB::table('bookingsource')->where('propertyid', $this->propertyid)->orderBy('name', 'ASC')->get();
    }
    public function download() {
        $data = $this->getData();
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Booking Source');
        ...
        $sheet->mergeCells('A2:D2'); $sheet->setCellValue('A2', 'Booking Source');
        $sheet->getStyle('A2')->applyFromArray(['font' => ['bold' => true, 'size' => 11], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER]]);
        $currentRow = 4;
        foreach (['Sn.', 'Name', 'Defined', 'Active'] as $i => $h) { $sheet->setCellValue(['A','B','C','D'][$i] . $currentRow, $h); }
        ...
        foreach ($data as $i => $row) {
            ...
            $sheet->setCellValue('C' . $currentRow, ($row->sysYN ?? '') == 'Y' ? 'System' : 'User');
            $sheet->setCellValue('D' . $currentRow, ($row->activeYN ?? '') == 'Y' ? 'Yes' : 'No');
        ...
        header('Content-Disposition: attachment; filename="Booking_Source.xlsx"');
