...
class NcTypeMasterExport {
    ...
    public function getData() {
        return DB::table('nctype_mast')->where('propertyid', $this->propertyid)->orderBy('nctype', 'ASC')->get();
    }
    public function download() {
        $data = $this->getData();
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('NC Type Master');
        ...
        foreach ($data as $i => $row) {
            ...
            $sheet->setCellValue('B' . $currentRow, $row->nctype ?? '');
            $sheet->setCellValue('C' . $currentRow, $row->ncper  ?? '');
        ...
        header('Content-Disposition: attachment; filename="NC_Type_Master.xlsx"');
