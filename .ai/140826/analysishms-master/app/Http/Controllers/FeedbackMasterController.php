...
class FeedbackMasterController extends Controller
{
    ...
    public function index($propertyid, $roomno)
    {
        ...
        $serviceItemsRaw = DB::table('hkamentiesmaster as H')
            ...
            ->select('H.type', 'I.Name', 'I.Code as itemcode')   // ← itemcode added
        ...
        $serviceItems = $serviceItemsRaw->groupBy('type')->map(function ($group) {
            return $group->map(function ($item) {
                return ['name' => $item->Name, 'itemcode' => $item->itemcode];
            })->values();
        });
        ...
        if ($serviceItems->isEmpty()) {
            $serviceItems = collect([
                'Linen'     => collect([
                    ['name' => 'Bedsheet', 'itemcode' => null],
                    ['name' => 'Pillow', 'itemcode' => null],
                    ['name' => 'Blanket', 'itemcode' => null],
                    ['name' => 'Towel', 'itemcode' => null],
                ]),
                'Amenities' => collect([
                    ['name' => 'Shampoo', 'itemcode' => null],
                    ['name' => 'Soap', 'itemcode' => null],
                    ['name' => 'Slippers', 'itemcode' => null],
                    ['name' => 'Drinking Water', 'itemcode' => null],
                ]),
            ]);
        }
    ...
    public function serviceRequest(Request $request, $propertyid, $roomno)
    {
        $request->validate([
            ...
            'items.*.itemcode'  => 'nullable|string|max:20',
            'items.*.qty'       => 'nullable|numeric|min:1',
            'notes'             => 'nullable|string|max:35',
        ]);

        // ... roomData fetch same as pehle ...
        ...
        try {
            ...
            foreach ($request->input('items') as $it) {
                DB::table('servicerequesthdrdtl')->insert([
                    ...
                    'requestno'   => $newRequestNo,
                    'sno'         => $sno,
                    'itemname'    => \Illuminate\Support\Str::limit($it['item'], 15, ''),
            ...
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Could not save your request. Please try again.']);
        }
    }
    ...
    public function acceptServiceRequest(Request $request)
    {
        ...
        try {
            ...
            date_default_timezone_set('Asia/Kolkata');
            $now   = now();
            $vdate = DB::table('enviro_general')->where('propertyid', $propertyid)->value('ncur');
            ...
            // Sirf Amenities ka stock minus hoga — Linen skip (jaisa HK cleaning module mein hai)
            $dtlRows = (clone $dtlQuery)
                ->where('requesttype', 'amenities')
            ...
            // ── HKISS Stock Voucher — same godown/vtype jo HK cleaning module use karta hai ──
            if ($dtlRows->isNotEmpty()) {
                $vtype    = 'HKISS';
                $hkGodown = 'HK' . $propertyid;
                $roomno   = $hdr->roomno ?? $request->roomno;
                ...
                if ($chkvpf) {
                    $vno     = \App\Models\Stock::where('propertyid', $propertyid)
                        ->where('vtype', $vtype)
                        ->lockForUpdate()
                        ->max('vno') + 1;
                    $vprefix = $chkvpf->prefix;
                    // NOTE: plain spaces use kiye — HK module mein invisible LRM chars the (known bug), yahan avoid kiya
                    $docid   = $propertyid . $vtype . '  ' . $vprefix . '    ' . $vno;

                    $stockRows = [];
                    $snoStock  = 1;

                    foreach ($dtlRows as $row) {
                        $qty   = (float) ($row->qty ?? 1);
                        $icode = $row->itemcode;
                        if ($qty <= 0 || empty($icode)) continue;
                        ...
                        $stockRows[] = [
                            ...
                            'remarks'      => 'Guest Portal Request #' . $request->requestno,
                            'u_name'       => $staffName,
                            'u_entdt'      => $now,
                            'u_ae'         => 'a',
                            'delflag'      => 'N',
