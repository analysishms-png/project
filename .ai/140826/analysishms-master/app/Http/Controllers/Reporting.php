...
class Reporting extends Controller
{
   ...
   public function fetchguesttraildata(Request $request)
   {
      ...
      foreach ($seqrevcode as $code) {
         $alias = "sum_" . strtolower($code);
         // $selectFields[] = DB::raw("SUM(CASE WHEN paycharge.paycode = '{$code}' and roomocc.type = 'O' THEN paycharge.amtdr ELSE 0 END) AS {$alias}");
         $selectFields[] = DB::raw("SUM(CASE WHEN paycharge.paycode = '{$code}' THEN paycharge.amtdr ELSE 0 END) AS {$alias}");
         $dynamicAliases[] = $alias;
      }
   ...
   public function fetchcontribuition(Request $request)
   {
    ...
    ")
         ...
         foreach ($data as $row) {
            $amountsum = Paycharge::where('propertyid', $this->propertyid)
               ->where('comp_code', $row->compcode)
    ...
    ")
            ->leftJoin('paycharge as p', function ($join) use ($startdate, $enddate) {
               $join->on('p.travel_agent', '=', 'sg.sub_code')
                  ->whereBetween('p.vdate', [$startdate, $enddate])
                  ->where('p.propertyid', $this->propertyid)
                  ->where('p.sno', '1')
                  ->where('p.paycode', 'RMCH' . $this->propertyid)
            ...
            // ->Join('guestfolio as GF', function ($join) {
            //    $join->on('p.folionodocid', '=', 'GF.docid')
            //       ->on('GF.travelagent', '=', 'sg.sub_code')
            //       ->where('GF.propertyid', $this->propertyid);
            // })
            ->where('sg.propertyid', $this->propertyid)
            ->where('sg.comp_type', $type)
         ...
         foreach ($data as $row) {
            $amountsum = Paycharge::where('paycharge.propertyid', $this->propertyid)
               ->leftJoin('guestfolio as GF', function ($join) use ($startdate, $enddate) {
                  $join->on('paycharge.folionodocid', '=', 'GF.docid')
                     ->where('GF.propertyid', $this->propertyid);
               })
               ->where('paycharge.travel_agent', $row->compcode)
    ...
    public function exportContributionExcel(Request $request)
    {
       try {
         ...
         ")
           ...
           $data = $data->filter(fn($r) => $r->revenue > 0);

           if ($data->isEmpty()) {
             return response()->json(['error' => 'No data found for the selected criteria.'], 404);
          }
          ...
          $row = 2;
          foreach ($data as $r) {
             $sheet->setCellValue('A' . $row, $r->company_name);

             for ($day = 1; $day <= 31; $day++) {
                $dayStr = str_pad($day, 2, '0', STR_PAD_LEFT);
                $colLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($day + 1);
                $sheet->setCellValueExplicit($colLetter . $row, (int)($r->$dayStr ?? 0), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
             }

             $totalCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(33);
             $revCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(34);

             $sheet->setCellValueExplicit($totalCol . $row, (int)($r->total_nights ?? 0), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
             $sheet->setCellValueExplicit($revCol . $row, (float)($r->revenue ?? 0), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
          ...
          $totalNightsCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(33);
          $revCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(34);

          $sheet->setCellValue($totalNightsCol . $totalRow, '=SUM(' . $totalNightsCol . '2:' . $totalNightsCol . ($totalRow - 1) . ')');
          $sheet->setCellValue($revCol . $totalRow, '=SUM(' . $revCol . '2:' . $revCol . ($totalRow - 1) . ')');

          $sheet->getStyle('A' . $totalRow . ':' . $revCol . $totalRow)->getFont()->setBold(true);

          for ($day = 1; $day <= 31; $day++) {
             $colLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($day + 1);
             $sheet->getStyle($colLetter . '2:' . $colLetter . ($totalRow - 1))->getNumberFormat()->setFormatCode('#,##0');
          }

          $sheet->getStyle($totalNightsCol . '2:' . $totalNightsCol . ($totalRow - 1))->getNumberFormat()->setFormatCode('#,##0');
          $sheet->getStyle($revCol . '2:' . $revCol . ($totalRow - 1))->getNumberFormat()->setFormatCode('#,##0.00');
          ...
          $filename = "Contribution_Report_{$type}_{$monthName}_{$year}_" . date('YmdHis') . '.xlsx';
   ...
   public function fetchcompanywiseanalysis(Request $request)
   {
      try {
         ...
         if ($type === 'Corporate') {
            // Exact match of original SQL — GROUP BY G.company only
            $data = DB::select("
               SELECT
                  S.name,
                  MAX(R.chkoutdate)                                         AS last_checkout,
                  SUM(R.nodays)                                             AS nights,
                  SUM(P.amtdr)                                              AS amount,
                  SUM(CASE WHEN P.paycode = ?  THEN P.amtdr ELSE 0 END)    AS room_rent,
                  SUM(CASE WHEN P.paycode = ?  THEN P.amtdr ELSE 0 END)    AS out_charge,
                  SUM(CASE WHEN P.paycode IN (?,?,?) THEN P.amtdr ELSE 0 END) AS tax
               FROM roomocc R
               INNER JOIN guestfolio G  ON R.docid = G.docid
               LEFT  JOIN subgroup   S  ON G.company = S.sub_code
               LEFT  JOIN paycharge  P  ON R.docid   = P.folionodocid
               WHERE R.propertyid = ?
                 AND R.chkoutdate BETWEEN ? AND ?
                 AND S.comp_type  = 'Corporate'
               GROUP BY G.company
               ORDER BY S.name
            ", [$rmch, $tout, $cgss, $sgss, $igss, $pid, $fromdate, $todate]);
         } else {
            // Travel Agency — GROUP BY G.company (travelagent column)
            $data = DB::select("
               SELECT
                  S.name,
                  MAX(R.chkoutdate)                                         AS last_checkout,
               ...
               FROM roomocc R
               INNER JOIN guestfolio G  ON R.docid = G.docid
               LEFT  JOIN subgroup   S  ON G.travelagent = S.sub_code
               LEFT  JOIN paycharge  P  ON R.docid = P.folionodocid
               WHERE R.propertyid = ?
                 AND R.chkoutdate BETWEEN ? AND ?
