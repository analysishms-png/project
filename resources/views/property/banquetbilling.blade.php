...
@section('main-container')
                            ...
                            <form id="banquetbillform" name="banquetbillform" action="{{ url('banquetbillingsubmit') }}"
                                ...
                                <div class="row">
                                    <div class="col-md-12 d-flex align-items-center" style="gap: 20px;">
                                        ...
                                        <div>
                                            ...
                                            <select class="form-control select2-multiple" name="oldbillno" id="oldbillno">
                                                ...
                                                @foreach (oldbanqutbillnos() as $col)
                                                    <option data-billno="{{ $col->vno }}" value="{{ $col->docId }}" {{ $col->docId == $docid ? 'selected' : '' }}>{{ $col->vno }} {{ $col->party }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div id="einvoicebtndiv">

                                        </div>

                                        <div id="einvoicecancelbtndiv">

                                        </div>

                                        <div id="viewinvoicebtndiv">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '#oldbillno', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        let deleteallow = response.deleteallow;
                        let venue = response.venues;
                        let sundrytype = response.sundrytype;
                        ...
                        $('#booking_date').attr('readonly', true); // Add Read only
                        ...
                        if (deleteallow == true) {
                            ...
                            $('#billdelete').prop('disabled', true);
                        }


                        $('#booking_no_span').text(hallbook.vno);

                        if (response.subgroup?.gstin && "{{ invoiceparameter()->activeyn == 'Y' }}") {
                        ...
                        let einvoicedata = response.einvoice;
                        ...
                        if (einvoicedata) {
                            ...
                            if (hoursDiff < 24) {
                                let invcancelbtn = `<button id="einvoicebtncancel" type="button" class="btn btn-sm btn-outline-danger">eInvoice Cancel <i
                            ...
                            setTimeout(() => {
                                overlayLock($('#totalpax, #paxrate, #company_name, .qtyisss, .rates, .amounts, .items, .discountfix, .discountsundry'), "Can't Change This");
                                $('.removerow').remove();
                            }, 1000);
                            ...
                            // $('#billdelete').prop('disabled', false);
                            overlayUnlock($('#totalpax, #paxrate, #company_name, .qtyisss, .rates, .amounts, .items, .discountfix, .discountsundry'));
                        }
            ...
            function getBanqRestCode() {
                return (restcode || localStorage.getItem('banqrestcode') || '').toString().trim();
            }

            function readNumberFrom($el) {
                const raw = ($el && typeof $el.val === 'function') ? $el.val() : '';
            ...
            function wtqty(accqty, index, rate) {
                ...
                const code = getBanqRestCode();
                if (code) {
                    $(`#s${code}totalamount`).text(sumofamounts('.amounts'));
                }
            }
            $(document).on('input', 'input[id$="discountfix"]', function() {
                const code = getBanqRestCode();
                if (!code || this.id !== `s${code}discountfix`) return;
            ...
            let disctime;
            $(document).on('input', 'input[id$="discountsundry"]', function() {
                const code = getBanqRestCode();
                if (!code || this.id !== `s${code}discountsundry`) return;
                ...
                disctime = setTimeout(() => {
                    ...
                    $(`#s${code}discountfix`).val(discountPercentage.toFixed(2));
                    setTimeout(() => {
                        calculateamt('amount');
                    }, 1000);
                }, 2000);
            ...
            function calculateamt(source = 'percent') {
                setTimeout(() => {
                    ...
                    let taxable = 0;
                    let nontaxable = 0;
                    let discountinput = readNumberFrom($(`#s${code}discountfix`));
                    let totalTaxAmount = 0;
                    $('input[data-srevcode]').val('0.00');
                    for (let i = 1; i <= tbodyLength; i++) {
                        let itemratewithoutqty = parseFloat($(`#itemrate${i}`).val()) || 0.00;
                        ...
                        if (trate > 0) {
                            taxable = itemratewithoutqty;
                            // console.log(taxable)
                            $(`#taxedrate${i}`).val(itemratewithoutqty);
                        } else {
                            nontaxable = itemratewithoutqty;
                    ...
                    let discountpercent = readNumberFrom($(`#s${code}discountfix`));
                    let discountamount = readNumberFrom($(`#s${code}discountsundry`));
                    $(`#s${code}totaltaxable`).val(taxable);
                    $(`#s${code}totalnontaxable`).val(nontaxable);
                    let totalamounts = sumofamounts('.amounts') || 0.00;
                    $(`#s${code}totalamountoutlet`).val(totalamounts.toFixed(2));
                    let totalAmount = readNumberFrom($(`#s${code}totalamountoutlet`));
                    if (source === 'percent') {
                        discountamount = (totalAmount * discountpercent) / 100;
                        $(`#s${code}discountsundry`).val(discountamount.toFixed(2));
                    } else if (source === 'amount') {
                        discountpercent = (totalAmount > 0) ? (discountamount / totalAmount) * 100 : 0;
                        $(`#s${code}discountfix`).val(discountpercent.toFixed(2));
                    }
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $(`#s${code}roundoffamount`).val(decimalvalue.toFixed(2));
                            $(`#s${code}netamount`).val(integervalue.toFixed(2));
                        },
                        error: function(err) {
                            console.error("Roundoff error:", err);
                        }
            ...
            function setfinalamounts() {
                setTimeout(() => {
                    const code = getBanqRestCode();
                    if (!code) return;
                    let firstnetamount = readNumberFrom($(`#${code}netamount`));
                    let secondnetamount = readNumberFrom($(`#s${code}netamount`));
            ...
            $(document).on('click', '#viewinvoicebtn', function() {
                ...
                if (einvoicedata) {
                    let invoice = JSON.parse(einvoicedata);
                    let irncode = invoice.irn || '';
                    let ackno = invoice.ackno || '';
                    let ackdt = invoice.ackdt || '';
                    let qrcodeimage = invoice.qrcodeimage || '';
                    let modalHtml = `
                        <div class="modal fade" id="einvoiceViewModal" tabindex="-1" aria-labelledby="einvoiceViewModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="einvoiceViewModalLabel">E-Invoice Details</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body text-center">
                                        <p><strong>IRN:</strong> ${irncode}</p>
                                        <p><strong>Ack No:</strong> ${ackno}</p>
                                        <p><strong>Ack Date:</strong> ${ackdt}</p>
                                        <img src="data:image/png;base64,${qrcodeimage}" alt="QR Code" class="img-fluid">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    $('#einvoiceViewModal').modal('show');
                    $('#einvoiceViewModal').on('hidden.bs.modal', function() {
                        $('#einvoiceViewModal').remove();
                    });
            ...
            $(document).on('click', '#einvoicebtncancel', function() {
                ...
                let docid = $('#oldhalldocid').val();
                let billno = $('#oldbillno').find(':selected').data('billno');
