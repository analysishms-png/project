...
<div class="container-fluid" style="margin-top:90px;">
   <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
         <h4 class="card-title mb-0"><i class="fas fa-home"></i> Instant House Count</h4>
      ...
      <div class="card-body">
         ...
         <table class="table table-sm table-bordered table-striped" id="rTable">
            <thead class="thead-dark"><tr><th class=" text-right">Total Rooms</th>
               <th class=" text-right">Occupied</th>
               <th class=" text-right">Vacant</th>
               <th class=" text-right">Blocked/OOO</th>
               <th class=" text-right">Pax In House</th>
               <th class=" text-right">Adults</th>
               <th class=" text-right">Exp. Arrivals Today</th>
               <th class=" text-right">Exp. Departures Today</th>
               </tr></thead>
...
$(document).ready(function() {
   ...
   $('#fetchBtn').click(function() {
      $.post('{{ route("inhousecountfetch") }}', {fromdate:$('#fromdate').val(),todate:$('#todate').val()
...
var h='';var tot=0;$.each(res.data,function(i,r){
            ...
            h+='<tr>'+'<td class="text-right">'+(r.total_rooms??0)+'</td>'+'<td class="text-right">'+(r.occupied??0)+'</td>'+'<td class="text-right">'+(r.vacant??0)+'</td>'+'<td class="text-right">'+(r.blocked??0)+'</td>'+'<td class="text-right">'+(r.pax??0)+'</td>'+'<td class="text-right">'+(r.adults??0)+'</td>'+'<td class="text-right">'+(r.expected_arrivals??0)+'</td>'+'<td class="text-right">'+(r.expected_departures??0)+'</td>'+'</tr>';});
         $('#tableBody').html(h);
