...
@section('main-container')
                ...
                <div class="col-lg-12">
                    <div class="card shadow report-card">
                        <div class="card-header bg-primary text-white">
                            <h5 class="mb-0">Cash Book / Bank Book</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="d-flex align-items-center justify-content-start mb-3 gap-2">
                                ...
                                <span id="printUrl" style="display:none;">{{ route('printcashbankbook') }}</span>
                            </div>
                            <div class="row report-toolbar align-items-center">
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <label for="bookFilter" class="col-form-label mb-0">Book</label>
                                        <select id="bookFilter" class="form-control">
                            ...
                            <div class="table-responsive">
                                <table id="cash-bank-table" class="table table-bordered table-striped table-hover">
                                    <thead class="bg-secondary text-white">
                                        <tr>
                                            <th style="width: 20%;">A/C Name</th>
                                            <th style="width: 9%;">Date</th>
                                            <th style="width: 13%;">Doc ID</th>
                                            ...
                                            <th class="text-right" style="width: 8%;">Balance</th>
                                        </tr>
                                    </thead>
                                    <tbody></tbody>
    ...
    <script>
        $(document).ready(function() {
            ...
            function getBook() {
                return $('#bookFilter').val() || 'Cash';
            }

            function loadAccounts() {
                const book = getBook();
                $.ajax({
                    url: '{{ route('cashbankbook.accounts') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        book: book
            ...
            function loadReport() {
                ...
                $('#cash-bank-table tbody').empty();
                $('#total-dr').text('0.00');
                $('#total-cr').text('0.00');

                $.ajax({
                    url: '{{ route('cashbankbook.fetch') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        book: book,
                    ...
                    success: function(response) {
                        ...
                        accounts.forEach(acc => {
                            ...
                            (acc.transactions || []).forEach(tx => {
                                html += `<tr class="txn-row">
                                    ...
                                    <td class="text-right">${formatNumber(tx.running_balance)}</td>
                                </tr>`;
                            });
                        ...
                        $('#cash-bank-table tbody').html(html);
            ...
            function exportToExcel() {
                ...
                let url = '{{ route('cashbankbook.export') }}'
                    + '?book=' + encodeURIComponent(getBook())
            ...
            $('#refreshButton').on('click', loadReport);
