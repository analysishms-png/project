                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    ...
                                    <div style="display: none;" id="onehalftablediv" class="table-responsive">
                                        <div class="mb-3 d-flex align-items-center flex-wrap gap-2 justify-content-around">
                                            <div class="form-group">
                                                <label for="fromdate" class="col-form-label">From Date <i class="fa-regular fa-calendar mb-1"></i></label>
                                                <input type="date" class="form-control fromdate" name="fromdatemhf" id="fromdatemhf">
                                            </div>
                                            <div class="form-group">
                                                <label for="todate" class="col-form-label">To Date <i class="fa-regular fa-calendar mb-1"></i></label>
                                                <input type="date" class="form-control todate" name="todatemhf" id="todatemhf">
                                            </div>
                                        </div>
                                        <table id="onehalftable-table" class="table table-bordered table-striped table-hover">
                                            <thead class="bg-black">
                                                <tr>
                                                    <th>Particular</th>
                                                    <th class="amountthhf">Amount</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                            ...
                                            <tfoot class="bg-light fw-bold">
                                                <tr>
                                                    <td>Total</td>
                                                    <td id="total-balancehf" class="text-end">0.00</td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetchtrialfirst() {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        let rightsumfirst = 0.00;
                        let leftsumfirst = 0.00;
                        ...
                        function getGroupMeta(groupKey) {
                            const group = groupsac?.[groupKey] || null;
                            const rows = groupsac?.[`${groupKey}_rows`] || [];
                            const firstRow = rows && rows.length ? rows[0] : null;
                            return {
                                name: group?.group_name || firstRow?.name || GROUP_LABELS[groupKey] || groupKey,
                                maingroupcode: group?.maingroupcode || firstRow?.maingroupcode || '',
                                group_code: group?.group_code || firstRow?.group_code || '',
                            };
                        }

                        function appendMainRow(tableSelector, groupKey, side) {
                            const rows = groupsac?.[`${groupKey}_rows`] || [];
                            const total = sumAbsBalances(rows);
                        ...
                        function appendSubRows(tableSelector, groupKey) {
                            if (!detailed) return;
                            ...
                            subgroups.forEach(subrow => {
                                const subBalance = toAmount(subrow.balance);
                                const hideClass = subBalance <= 0 ? 'd-none' : '';

                                const subtr = `<tr class="clickable-row ${hideClass}"
                                                style="background-color: #f0f0f0;"
                                                data-acgroupcode="${subrow.acgroupcode || ''}"
                                                data-maingroupcode="${meta.maingroupcode || ''}"
                                                data-group_code="${meta.group_code || ''}"
                                                data-undergroup="${subrow.undergroup || 'N'}"
                                                data-sub_code="${subrow.subcode || ''}"
                                                data-rowyear="${subrow.year || ''}"
                                                data-compname="${subrow.name || ''}">
                                                <td style="padding-left: 30px;">${subrow.name || ''}</td>
                                                <td class="amount-col">${subBalance.toFixed(2)}</td>
                                                <td class="amount-col"></td>
                                            </tr>`;
                        ...
                        // Right side (Credit)
                        rightsumfirst += appendMainRow('#right-table', 'closingstock', 'right');
                        appendSubRows('#right-table', 'closingstock');

                        rightsumfirst += appendMainRow('#right-table', 'sale', 'right');
                        appendSubRows('#right-table', 'sale');

                        rightsumfirst += appendMainRow('#right-table', 'directincome', 'right');
                        appendSubRows('#right-table', 'directincome');

                        // Left side (Debit)
                        leftsumfirst += appendMainRow('#left-table', 'openingstock', 'left');
                        appendSubRows('#left-table', 'openingstock');

                        leftsumfirst += appendMainRow('#left-table', 'purchaseaccount', 'left');
                        appendSubRows('#left-table', 'purchaseaccount');

                        leftsumfirst += appendMainRow('#left-table', 'directexpenses', 'left');
                        appendSubRows('#left-table', 'directexpenses');
                        ...
                        $('#left-table tbody').append(spacer);
                        $('#right-table tbody').append(spacer);

                        let profitcarryoverval = 0;
                        let losscarryoverval = 0;

                        if (rightsumfirst < leftsumfirst) {
                            // Right side is less, show Loss c/o on right
                            losscarryoverval = leftsumfirst - rightsumfirst;

                            let trloss = `
                                    <tr>
                                        <td><strong>Loss c/o</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="losscarryover" class="amount-col"><strong>${losscarryoverval.toFixed(2)}</strong></td>
                                    </tr>`;
                            $('#right-table tbody').append(trloss);

                            $('#left-table tbody').append(emptytr20px);
                        } else if (leftsumfirst < rightsumfirst) {
                            // Left side is less, show Profit c/o on left
                            profitcarryoverval = rightsumfirst - leftsumfirst;

                            let trprofit = `
                                    <tr>
                                        <td><strong>Profit c/o</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="profitcarryover" class="amount-col"><strong>${profitcarryoverval.toFixed(2)}</strong></td>
                                    </tr>`;
                            $('#left-table tbody').append(trprofit);

                            $('#right-table tbody').append(emptytr20px);
                        }

                        let leftsumfirsttotal = leftsumfirst + profitcarryoverval;
                        let rightsumfirsttotal = rightsumfirst + losscarryoverval;

                        let trtotalfirstleft = `
                                    <tr class="total-row">
                                        <td><strong>Total</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="totalamountleft" class="amount-col"><strong>${leftsumfirsttotal.toFixed(2)}</strong></td>
                                    </tr>`;

                        $('#left-table tbody').append(trtotalfirstleft);

                        let trtotalfirstright = `
                                    <tr class="total-row">
                                        <td><strong>Total</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="totalamountirhgt" class="amount-col"><strong>${rightsumfirsttotal.toFixed(2)}</strong></td>
                                    </tr>`;

                        $('#right-table tbody').append(trtotalfirstright);
                        ...
                        $('#left-table tbody').append(spacer2);
                        $('#right-table tbody').append(spacer2);

                        if (profitcarryoverval > 0) {
                            let trprofitbf = `
                                    <tr>
                                        <td><strong>Profit b/f</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="profitbf" class="amount-col"><strong>${profitcarryoverval.toFixed(2)}</strong></td>
                                    </tr>`;
                            $('#right-table tbody').append(trprofitbf);
                            $('#right-table tbody').append(emptytr20px);
                        } else if (losscarryoverval > 0) {
                            let trlossbf = `
                                    <tr>
                                        <td><strong>Loss b/f</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="lossbf" class="amount-col"><strong>${losscarryoverval.toFixed(2)}</strong></td>
                                    </tr>`;
                            $('#right-table tbody').append(trlossbf);
                            $('#left-table tbody').append(emptytr20px);
                        }

                        const indirectExpensesTotal = appendMainRow('#left-table', 'indirectexpenses', 'left');
                        appendSubRows('#left-table', 'indirectexpenses');

                        const indirectIncomeTotal = appendMainRow('#right-table', 'indirectincome', 'right');
                        appendSubRows('#right-table', 'indirectincome');
                        ...
                        $('#left-table tbody').append(spacer3);
                        $('#right-table tbody').append(spacer3);

                        let chkIncome = indirectIncomeTotal + profitcarryoverval - indirectExpensesTotal;
                        let netProfit = chkIncome > 0 ? chkIncome : 0;
                        let netLoss = chkIncome < 0 ? Math.abs(chkIncome) : 0;
                        let finaltotalright = indirectIncomeTotal + profitcarryoverval + netLoss;
                        let finaltotalleft = indirectExpensesTotal + netProfit;

                        if (netProfit > 0) {
                            let trnetfinalprofit = `
                                    <tr>
                                        <td><strong>Net Profit</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="netprofitval" class="amount-col"><strong>${netProfit.toFixed(2)}</strong></td>
                                    </tr>`;
                            $('#left-table tbody').append(trnetfinalprofit);
                            $('#right-table tbody').append(emptytr20px);
                        } else if (netLoss > 0) {
                            let trnetfinalloss = `
                                    <tr>
                                        ...
                                        <td class="amount-col"></td>
                                        <td id="netlossval" class="amount-col"><strong>${netLoss.toFixed(2)}</strong></td>
                                    </tr>`;
                            $('#right-table tbody').append(trnetfinalloss);
                            $('#left-table tbody').append(emptytr20px);
                        }


                        let trtotalfinalright = `
                                    <tr class="total-row">
                                        <td><strong>Total</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="totalrightfinal" class="amount-col"><strong>${finaltotalright.toFixed(2)}</strong></td>
                                    </tr>`;
                        $('#right-table tbody').append(trtotalfinalright);
                        $('#left-table tbody').append(emptytr20px);
                        $('#left-table tbody').append(emptytr20px);
                        let trtotalfinalleft = `
                                    <tr class="total-row">
                                        <td><strong>Total</strong></td>
                                        <td class="amount-col"></td>
                                        <td id="totalleftfinal" class="amount-col"><strong>${finaltotalleft.toFixed(2)}</strong></td>
                                    </tr>`;
                        $('#left-table tbody').append(trtotalfinalleft);
                    },
                    error: function(xhr) {
                        hideLoader();
            ...
            $(document).on('click', '#exportbutton', function() {
                ...
                let headerLines = buildExportHeaderLines();
                ...
                if (isDetailed) {
                    allData.push(['Particulars (Debit/Expenses)', 'Sub Amount', 'Total Amount', 'Particulars (Credit/Income)', 'Sub Amount', 'Total Amount']);
                } else {
                    allData.push(['Particulars (Debit/Expenses)', 'Amount', 'Particulars (Credit/Income)', 'Amount']);
                }
                allData.push([]);
                ...
                // Add footer
                allData.push([]);
                allData.push(['Prepared By: {{ Auth::user()->name }}']);
                allData.push(['Date: {{ ncurdate() }}']);
                ...
                // Generate filename
                let filename = 'ProfitLoss_' + fromDate + '_to_' + toDate + '.xlsx';
            ...
            let dataTableInitialized7 = false;

            function fetchonehalfdata(fromdateId, todateId) {
                ...
                const group_code = selectedRow.data('acgroupcode');
                $('#companyname').text(companyname);
                $('#onehalftable-table tbody').empty();
                $('#onehalftablediv').fadeOut('500');
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('profitlosssecondqueryhf') }}",
                    data: {
                        _token: $('meta[name="csrf-token"]').attr('content'),
                        ...
                        openingbalance: $('#openingbalance').is(':checked') ? 'checked' : 'not checked',
                    ...
                    success: function(response) {
                        ...
                        response.subgroups.forEach(row => {
                            ...
                            rows += `<tr class="clickable-row"
                                        ...
                                        <td>${row.name}</td>
                                        <td class="text-end amounthf">${amount}</td>
                                    </tr>`;
                        });

                        $('#total-balancehf').text(totalAmount.toFixed(2));

                        $('#onehalftable-table tbody').html(rows);
                        $('#onehalftablediv').hide().removeClass('d-none').fadeIn(300);

                        if (!dataTableInitialized7) {
                            $('#onehalftable-table').DataTable({
                                dom: 'Bfrtip',
                                ordering: true,
                                order: [],
                                buttons: [{
                    ...
                    success: function(response) {
                        ...
                        response.subgroups.forEach(row => {
                            ...
                            rows += `<tr class="clickable-row ${hideClass}"
                                        ...
                                        data-sub_code="${row.subcode}"
                                        data-compname="${row.name}"
                                        data-undergroup="${row.undergroup}"
                                        data-rowyear="${row.year}">
                                        <td>${row.name}</td>
            ...
            $(document).on('click', '.clickable-row', function() {
                ...
                console.log('underout');
                if ($(this).data('undergroup') == 'N') {
                    fetchonehalfdata('fromdate', 'todatem');
                    console.log('undercoming');
                    return;
                }
                // if (groupynvalue == 0) {
                fetchTrialData('fromdatem', 'todatem');
            ...
            // Build export header lines for consistent formatting
            ...
            // Build export header HTML for consistent formatting
            ...
            // Add header rows to Excel sheet
            ...
            $(document).on('click', '#printbutton', function() {
                ...
                let isDetailed = $('.subrowsamt').is(':visible');
                let headerLines = buildExportHeaderLines();
                let headerHtml = buildExportHeaderHtml(headerLines);
                ...
                let htmlContent = `
                    ...
                    <body>
                        ...
                        <div class="print-footer">
                            ...
                            <p>Prepared By: {{ Auth::user()->name }} | Date: {{ ncurdate() }}</p>
                        </div>
                    </body>
                    </html>
                `;
