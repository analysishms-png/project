                    ...
                    <div class="card shadow report-card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex align-items-center justify-content-start mb-3 gap-2">
                                ...
                                <span id="printUrl" style="display:none;">{{ route('printjournalbook') }}</span>
                            </div>
                            <div class="row report-toolbar align-items-center">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label for="vtypeFilter" class="col-form-label mb-0">Voucher Type</label>
                                        <select id="vtypeFilter" class="form-control">
                                            <option value="JV" selected>JV — Journal</option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="table-responsive">
                                <table id="journal-book-table" class="table table-bordered table-striped table-hover">
    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        (response.data || []).forEach(vt => {
                            sel.append(`<option value="${vt}" ${vt === current ? 'selected' : ''}>${vt}</option>`);
                        });
                    }
                });
            }

            function loadJournalBook() {
                ...
                $('#journal-book-table tbody').empty();
                $('#total-dr').text('0.00');
                $('#total-cr').text('0.00');

                $.ajax({
                    url: '{{ route('journalbook.fetch') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        fromdate: fromdate,
                        todate: todate,
                        vtype: $('#vtypeFilter').val() || 'JV'
                    },
                    success: function(response) {
                        ...
                        $('#journal-book-table tbody').html(html);
            ...
            function exportToExcel() {
                ...
                let url = '{{ route('journalbook.export') }}'
            ...
            $('#refreshButton').on('click', loadJournalBook);
