...
<div class="container-fluid" style="margin-top:90px;">
   <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
         <h4 class="card-title mb-0"><i class="fas fa-glass-cheers"></i> Hall / Banquet Tax Report</h4>
      ...
      <div class="card-body">
         ...
         <table class="table table-sm table-bordered table-striped w-100" id="rTable">
            <thead class="thead-dark"><tr><th>Date</th><th>Bill No</th><th>Party</th><th>Description</th><th>Rate %</th><th class="text-right">Base</th><th class="text-right">CGST</th><th class="text-right">SGST</th><th class="text-right">IGST</th><th class="text-right">Total Tax</th><th class="text-right">Net</th></tr></thead>
...
$(document).ready(function() {
   ...
   var rows=[];
   function fetch() {
      $.post('{{ route("taxreporthallfetch") }}', {fromdate:$('#fromdate').val(),todate:$('#todate').val()
      ...
      , function(res) {
         ...
         $.each(rows,function(i,r){
            h+='<tr><td>'+fmtDate(r.vdate)+'</td><td>'+(r.billno||'')+'</td><td>'+(r.party||'-')+'</td><td>'+(r.descr||'')+'</td><td class="text-right">'+fmt(r.taxper)+'</td><td class="text-right">'+fmt(r.base)+'</td><td class="text-right">'+fmt(r.cgst)+'</td><td class="text-right">'+fmt(r.sgst)+'</td><td class="text-right">'+fmt(r.igst)+'</td><td class="text-right">'+fmt(r.tax)+'</td><td class="text-right"><b>'+fmt(r.net)+'</b></td></tr>';
         });
