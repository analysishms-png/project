...
<div class="container-fluid" style="margin-top:90px;">
   <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
         <h4 class="card-title mb-0"><i class="fas fa-utensils"></i> Tax-wise Sale (POS)</h4>
      ...
      <div class="card-body">
         ...
         <table class="table table-sm table-bordered table-striped w-100" id="rTable">
            <thead class="thead-dark"><tr><th>Outlet</th><th>Tax Head</th><th>Rate %</th><th class="text-right">Bills</th><th class="text-right">Base Value</th><th class="text-right">Tax Amount</th></tr></thead>
            <tbody id="tableBody"></tbody>
            <tfoot id="tFoot" style="display:none"><tr class="font-weight-bold table-secondary"><td colspan="4" class="text-right">TOTAL</td><td id="fBase" class="text-right">0</td><td id="fTax" class="text-right">0</td></tr></tfoot>
         </table>
      </div>
   </div>
</div>
...
$(document).ready(function() {
   ...
   var rows=[];
   function fetch() {
      $.post('{{ route("taxwisesalefetch") }}', {fromdate:$('#fromdate').val(),todate:$('#todate').val()
      ...
      , function(res) {
         ...
         $.each(rows,function(i,r){
            h+='<tr><td>'+(r.outlet||'-')+'</td><td>'+(r.taxhead||'')+'</td><td class="text-right">'+fmt(r.taxper)+'</td><td class="text-right">'+(r.bills??0)+'</td><td class="text-right">'+fmt(r.base)+'</td><td class="text-right"><b>'+fmt(r.taxamt)+'</b></td></tr>';
         });
         $('#tableBody').html(h||'<tr><td colspan=6 class="text-center text-muted">No records found</td></tr>');
