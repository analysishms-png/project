@include('property.print._header', [
    'company'  => $company,
    'title'    => ($book === 'Cash' ? 'Cash Book' : 'Bank Book') . ' Report',
    'fromDate' => $fromDate,
    'toDate'   => $toDate,
])
...
<table>
    <thead>
        <tr>
            ...
            <th style="width:14%;">Doc ID</th>
            <th style="width:26%;">Narration</th>
            <th style="width:9%;">Contra</th>
        ...
        @else
            ...
            @foreach ($accounts as $acc)
                ...
                @if (count($acc['transactions']) === 0)
                    <tr>
                        <td colspan="8" style="padding:4px; color:#6c757d; font-style:italic;">No transactions in period.</td>
                    </tr>
                @else
                    @foreach ($acc['transactions'] as $tx)
                        <tr>
                            ...
                            <td class="text-right">{{ number_format($tx['running_balance'], 2) }}</td>
                        </tr>
                    @endforeach
                @endif
