...
<div class="container-fluid" style="margin-top:90px;">
   <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
         <h4 class="card-title mb-0"><i class="fas fa-file-invoice-dollar"></i> Tax Details</h4>
      ...
      <div class="card-body">
         ...
         <table class="table table-sm table-bordered table-striped w-100" id="rTable">
            <thead class="thead-dark"><tr><th>Date</th><th>Bill No</th><th>Source</th><th>Party / Outlet</th><th>Rate %</th><th class="text-right">Base</th><th class="text-right">CGST</th><th class="text-right">SGST</th><th class="text-right">IGST</th><th class="text-right">Tax</th><th class="text-right">Net</th></tr></thead>
            <tbody id="tableBody"></tbody>
            <tfoot id="tFoot" style="display:none"><tr class="font-weight-bold table-secondary"><td colspan="5" class="text-right">TOTAL</td><td id="fBase" class="text-right">0</td><td id="fCgst" class="text-right">0</td><td id="fSgst" class="text-right">0</td><td id="fIgst" class="text-right">0</td><td id="fTax" class="text-right">0</td><td id="fNet" class="text-right">0</td></tr></tfoot>
         </table>
      </div>
   </div>
</div>
...
$(document).ready(function() {
   ...
   var rows=[];
   function fetch() {
      $.post('{{ route("taxdetailsfetch") }}', {fromdate:$('#fromdate').val(),todate:$('#todate').val()
      ...
      , function(res) {
         ...
         $.each(rows,function(i,r){
            h+='<tr><td>'+fmtDate(r.vdate)+'</td><td>'+(r.billno||'')+'</td><td><span class="badge badge-info">'+r.src+'</span></td><td>'+(r.party||'-')+'</td><td class="text-right">'+fmt(r.taxper)+'</td><td class="text-right">'+fmt(r.base)+'</td><td class="text-right">'+fmt(r.cgst)+'</td><td class="text-right">'+fmt(r.sgst)+'</td><td class="text-right">'+fmt(r.igst)+'</td><td class="text-right">'+fmt(r.tax)+'</td><td class="text-right"><b>'+fmt(r.net)+'</b></td></tr>';
         });
         $('#tableBody').html(h||'<tr><td colspan=11 class="text-center text-muted">No records found</td></tr>');
