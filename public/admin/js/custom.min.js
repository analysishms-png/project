...
//     if (guestEl) guestEl.value = nameValue; // Check if the guestEl element exists before setting its value
...
function PasteToModal() {
    ...
    const guestEl = document.getElementById("guestname");
    if (guestEl) guestEl.value = nameEl?.value ?? '';

    const greetingsEl = document.getElementById("greetings");
    const greetingsGuest = document.getElementById("greetingsguest");
    if (greetingsEl && greetingsGuest) greetingsGuest.value = greetingsEl.value;

    const mobileEl = document.getElementById("mobile");
    const guestMobile = document.getElementById("guestmobile");
    if (mobileEl && guestMobile) guestMobile.value = mobileEl.value;

    const emailEl = document.getElementById("email");
    const guestEmail = document.getElementById("guestemail");
    // if (emailEl && guestEmail) guestEmail.value = emailEl.value;

    const cityEl = document.getElementById("cityname");
    const cityGuest = document.getElementById("cityguest");
    const arrFrom = document.getElementById("arrfrom");
    const destination = document.getElementById("destination");
    if (cityEl) {
        const cityValue = cityEl.value;
        if (cityGuest) cityGuest.value = cityValue;
        if (arrFrom) arrFrom.value = cityValue;
        if (destination) destination.value = cityValue;
    }

    const stateEl = document.getElementById("state");
    const guestState = document.getElementById("stateguest");
    if (stateEl && guestState) {
    ...
    const guestZip = document.getElementById("zipguest");
    if (zipEl && guestZip) guestZip.value = zipEl.value;

    const nationalityEl = document.getElementById("nationality");
    const guestNationality = document.getElementById("nationalityother");
    if (nationalityEl && guestNationality) {
        const nationalityValue = nationalityEl.value ?? '';
...
function PasteToModalnum(index) {
    ...
    const guestNameEl = document.getElementById("guestname" + index);
    const nameValue = nameEl?.value ?? '';
    if (guestNameEl) guestNameEl.value = nameValue;

    // Greetings
    const greetingsEl = document.getElementById("greetings" + index);
    const greetingsGuestEl = document.getElementById("greetingsguest" + index);
    if (greetingsEl && greetingsGuestEl)
        greetingsGuestEl.value = greetingsEl.value;

    // Mobile
    const mobileEl = document.getElementById("mobile" + index);
    const guestMobileEl = document.getElementById("guestmobile" + index);
    if (mobileEl && guestMobileEl)
        guestMobileEl.value = mobileEl.value;

    // Email
    const emailEl = document.getElementById("email" + index);
    const guestEmailEl = document.getElementById("guestemail" + index);
    if (emailEl && guestEmailEl)
        guestEmailEl.value = emailEl.value;

    // City
    const cityEl = document.getElementById("cityname" + index);
    if (cityEl) {
        const cityValue = cityEl.value;

        const destinationEl = document.getElementById("destination" + index);
        const arrFromEl = document.getElementById("arrfrom" + index);
        const cityGuestEl = document.getElementById("cityguest" + index);

        if (destinationEl) destinationEl.value = cityValue;
        if (arrFromEl) arrFromEl.value = cityValue;
        if (cityGuestEl) cityGuestEl.value = cityValue;
    }

    // State
    const stateEl = document.getElementById("state" + index);
    const guestStateEl = document.getElementById("stateguest" + index);
    if (stateEl && guestStateEl) {
        const stateText = stateEl.options[stateEl.selectedIndex]?.text ?? '';
        const stateValue = stateEl.value ?? '';
        guestStateEl.innerHTML = `<option value="${stateValue}">${stateText}</option>`;
    }

    // Country
    const countryEl = document.getElementById("country" + index);
    const guestCountryEl = document.getElementById("countryguest" + index);
    if (countryEl && guestCountryEl) {
        const countryText = countryEl.options[countryEl.selectedIndex]?.text ?? '';
        const countryValue = countryEl.value ?? '';
        guestCountryEl.innerHTML = `<option value="${countryValue}">${countryText}</option>`;
    }

    // Zipcode
    const zipEl = document.getElementById("zipcode" + index);
    const guestZipEl = document.getElementById("zipguest" + index);
    if (zipEl && guestZipEl)
        guestZipEl.value = zipEl.value;

    // Nationality
    const nationalityEl = document.getElementById("nationality" + index);
    const guestNationalityEl = document.getElementById("nationalityother" + index);
    if (nationalityEl && guestNationalityEl) {
        const nationalityValue = nationalityEl.value ?? '';
        const nationalityText = nationalityEl.options[nationalityEl.selectedIndex]?.text ?? '';
        guestNationalityEl.innerHTML = `<option value="${nationalityValue}">${nationalityText}</option>`;
    }
}
...
function updateIndiaTime() {
    //console.log(getCurrentTimeIndia());
    timeout = setTimeout(updateIndiaTime, 1000);
}

updateIndiaTime();


// if you ever want to stop it, call:
...
// function validateFinancialYearmain(dateInputSelector) {
//     let dateInput = $(dateInputSelector);
